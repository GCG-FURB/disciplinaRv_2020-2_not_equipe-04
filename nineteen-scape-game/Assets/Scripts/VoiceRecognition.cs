...
public class VoiceRecognition : MonoBehaviour
{
    ...
    private string standardText = "meu nome é";
    ...
    public bool Active
    {
        ...
        set
        {
            ...
            {
                _service.RecognizeModel = (string.IsNullOrEmpty(_recognizeModel) ? "pt-BR_BroadbandModel" : _recognizeModel);
    ...
    {
        if (this.recognitionEnabled && !this.recognized)
        {
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        ResultsField.text = this.UsernameRecognition(text);
                    }
    ...
    private string UsernameRecognition(string text)
    {
        string aux = text.ToLower();
        
        try
        {
            if (aux.Contains(this.standardText)) 
            {
                aux = text.Substring(aux.IndexOf(this.standardText) + this.standardText.Length);
                ...
                aux = char.ToUpper(aux[0]) + aux.Substring(1);
